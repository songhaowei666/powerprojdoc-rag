...
- `src.config.settings`：读取 `chat_model`、`embedding_model`、`openai_api_key`、`openai_api_base`
- `src.generage_graph.build_llm`：构建评估器所用的 ChatOpenAI 实例
- `build_evaluator_embeddings()`：构建评估器所用的 OpenAIEmbeddings 实例（与 RAG 共用同一套 API Key / Base URL）
...
详细说明见 [`eval/metrics.md`](../eval/metrics.md#31-page_recallk页面召回率)。
...
详细计算说明见 [`eval/metrics.md`](../eval/metrics.md)。
...
**`run_batch` 执行流程**（逐条评估，默认通过 CLI `--limit 1` 每次只评一条）：

1. **切片样本**：按 `offset` / `limit` 取子集；`limit=None` 时评剩余全部
2. **逐条循环**（每条独立完成 RAG + ragas，避免一次性消耗大量 token）：
   - 调用 `rag_app.run(question, company_code)` 获取检索与生成结果
   - RAG 失败：该条 ragas 指标记为 NaN，`page_recall@k` 为 0.0（有 expected_pages 时）
   - RAG 成功：调用 `SingleTurnEvaluator.evaluate()` 计算 ragas 五指标与 `page_recall@k`
3. **输出 DataFrame**：列名包含
...
**ragas 指标**：使用 `ragas.metrics._*` 模块中已实例化的标准指标对象，由 `build_ragas_metrics()` 组装；`evaluate()` 同时注入 `evaluator_llm` 与 `evaluator_embeddings`（后者供 `answer_relevancy`、`answer_correctness` 等语义相似度指标使用，与 RAG 共用 `settings` 中的 API 配置）。

**边界行为**：
- 若单条 ragas 评估失败，抛出 `RuntimeError`，并附带样本索引
- 若某条样本的 `expected_answer` 为空字符串，不计算 `context_recall` 与 `answer_correctness`
...
| `--limit` | `1` | 本次评估样本数；`0` 表示评估全部 |
| `--offset` | `0` | 起始样本索引（0-based） |
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v1.5 | 2026-06-25 | ragas 评估显式注入 `build_evaluator_embeddings()`，与 RAG 共用 API Key / Base URL |
| v1.4 | 2026-06-24 | ragas 0.4.x 指标改为实例化对象；`run_batch` 改为逐条 RAG+ragas；CLI 新增 `--limit`/`--offset`（默认每次评 1 条） |
